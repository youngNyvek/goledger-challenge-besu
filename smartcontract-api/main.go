...
import (
	...
	ethereum "goledger-challenge-besu/internal/infra"
...
func main() {
	...
	router := gin.Default()
	ctrl := contract_controller.NewContractController(contract, client, signer)

	apiGroup := router.Group("simple-storage/v1/api/")
	apiGroup.POST("/setValue", ctrl.SetValue)
	apiGroup.GET("/getValue", ctrl.GetValue)
